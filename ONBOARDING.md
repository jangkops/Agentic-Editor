...
- **릴리스 파이프라인**: 2026-09-17 수동 실행(`publish=never`)에서 macOS 러너가 처음으로 끝까지 성공했고, Windows 러너는 fastembed 모델 캐시의 심볼릭 링크 때문에 7-Zip 패키징에서 실패했습니다. 2026-09-22에 `scripts/build-python.js`가 링크를 실제 파일로 풀고 고아 blobs를 지우도록 고친 뒤 **두 러너 모두 끝까지 성공**했습니다(7-Zip 경고 10건 → 0건). 같은 날 빌드 로그로 번들 모델 캐시 240MB, 남은 blobs 0, 오프라인 실로드 OK를 두 러너에서 확인했고, 산출물은 mac DMG/ZIP 각 약 480MB, Windows 설치 파일 약 450MB입니다. 아직 GitHub Release로 공개한 적은 없고, 자동 업데이트 배선이 다음 항목입니다.
