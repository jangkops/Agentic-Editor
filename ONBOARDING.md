...
- **릴리스 파이프라인**: 2026-09-17 수동 실행(`publish=never`)에서 **macOS 러너의 PyInstaller 동결 + DMG 빌드가 처음으로 끝까지 성공**했습니다. Windows 러너는 fastembed 모델 캐시의 심볼릭 링크 때문에 7-Zip 패키징에서 실패해, `scripts/build-python.js`에서 링크를 실제 파일로 풀어 주는 수정이 남아 있습니다.
