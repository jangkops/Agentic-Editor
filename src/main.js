...
// SSE 스트림 읽기 공통 함수
async function readSSEStream(resp, onData, onError) {
  ...
  while (true) {
    ...
    for (const event of events) {
      ...
      if (!trimmed) continue;
      // 여러 줄일 수 있으므로 첫 번째 data: 이후 전체를 텍스트로
      if (trimmed.startsWith('data: ')) {
        ...
        try {
          ...
          if (parsed.slotId) { onData(parsed, true); continue; } // 병렬 결과
          onData(d, false);
        } catch {
          // JSON이 아니면 텍스트 그대로
          onData(d, false);
...
function renderTerminalContent() {
  ...
  // 통합 입출력: 출력 아래에 커서가 바로 이어지는 형태
  c.innerHTML = `<div id="terminal-body" class="terminal-body">${esc(term.output)}<span id="terminal-cursor" class="terminal-cursor"></span></div>`;
  const body = c.querySelector('#terminal-body');
  // 클릭하면 포커스
  body.setAttribute('tabindex', '0');
  body.focus();
  body.addEventListener('keydown', e => {
    if (e.key === 'Enter') {
      const line = body.dataset.currentLine || '';
      term.output += line + '\n';
      body.dataset.currentLine = '';
      if (window.electronAPI?.terminalWrite) window.electronAPI.terminalWrite(term.id, line + '\n');
      renderTerminalOutput();
    } else if (e.key === 'Backspace') {
      const line = body.dataset.currentLine || '';
      if (line.length > 0) {
        body.dataset.currentLine = line.slice(0, -1);
        renderTerminalOutput();
      }
    } else if (e.key.length === 1 && !e.ctrlKey && !e.metaKey) {
      body.dataset.currentLine = (body.dataset.currentLine || '') + e.key;
      renderTerminalOutput();
    }
    e.preventDefault();
  });
...
function renderTerminalOutput() {
  ...
  if (!body || !term) return;
  const line = body.dataset.currentLine || '';
  body.innerHTML = esc(term.output) + esc(line) + '<span class="terminal-cursor"></span>';
  body.scrollTop = body.scrollHeight;
}

function appendTerminalOutput(text) {
  const term = state.terminals[state.activeTerminalIdx];
  if (term) { term.output += text; renderTerminalOutput(); }
}
...
function setupTerminalIPC() {
  if (window.electronAPI?.onTerminalData) {
    window.electronAPI.onTerminalData(data => {
      ...
      if (term) { term.output += data.data; if (term === state.terminals[state.activeTerminalIdx]) renderTerminalOutput(); }
    });
  }
}
